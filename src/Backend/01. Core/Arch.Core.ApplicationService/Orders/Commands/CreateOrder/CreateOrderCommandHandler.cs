...
public class CreateOrderCommandHandler : CommandHandler<CreateOrderCommand>
{
    ...
    {
        var entity = new Order(Guid.NewGuid(), command.UserId, (command.Street, command.Plaque), (command.TaxAmount, command.TaxRate), command.OrderDetail.ProductId, command.OrderDetail.Count);
